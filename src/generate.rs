use crate::constant::{DNS_CONTENT, FILTER_LOCAL, GENERAL_CONTENT, OTHER_SETTING};
...
fn generate_filter_remote(rule_list: &Vec<&str>) -> Vec<String> {
    ...
    for rule in rule_list {
        ...
        let policy_info: String = format!(
            "{}, tag={}, update-interval=172800, opt-parser=false, enabled=true",
            url, rule
        );
...
pub fn output_config_file_content(rule_list: Vec<&str>, node_names: String, node_list: String) {
    ...
    let policy_info = generate_policy_info(&rule_list, node_names);
    ...
    let mut node_list_info: Vec<&str> = vec!["[server_local]"];
