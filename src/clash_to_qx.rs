...
fn convert_protocal_name(clash_name: String) -> String {
    if clash_name == "ss" {
        String::from("shadowsocks")
    } else if clash_name == "ssr" {
        String::from("shadowsocksr")
    } else if clash_name == "vmess" {
        String::from("vmess")
    } else if clash_name == "trojan" {
        String::from("trojan")
    } else {
        String::from("unknown")
    }
...
pub fn format_proxies(proxies: Vec<Proxy>) -> Vec<String> {
    proxies
        ...
        .map(|proxy| {
            ...
            parts.push(format!(
                "{} = {}:{}",
                convert_protocal_name(proxy.proxy_type),
                proxy.server,
                proxy.port
            ));
